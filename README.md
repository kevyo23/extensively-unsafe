...
DISCLAIMER:
This is the outcome of a web security course self-directed project. It is not intended for public usage as there are deliberate holes and vulnerabilities introduced that may expose you to XSS. Use at your own risk.

This involves the following:

1. Creating a browser extension with the intention of providing greater visibility for extensions. It directs the user to their local store of extension (following the patternfile:///Users/[USR_DIR]/Library/
Application%20Support/[BRSR_NAM]/Default/Extensions) and performs analysis on all extension ID’s listed on the page, by calling Duo Lab’s crxcavator API, returning a tabulated format including extension risk, permission, dangerous functions and entry-points, and external calls made

2. Inserting rogue code (in the sense that it doesn’t behave as expected or claimed) into the extension, which extracts sensitive banking transaction details from a dummy site (ie. http://60764.mykidsbank.org/) that I visit, demonstrating potential for siphoned data to be sent to remote locations

3. Attempt different methods to mimic an XSS attack from a malicious site that takes advantage of the vulnerabilities exposed by how I wrote my browser extension (I also tried loosening type and format checks internally in the extension to allow for such attacks to occur more easily)


## This is built on top of TypeScript / React / Webpack / Chrome Extension Boilerplate
